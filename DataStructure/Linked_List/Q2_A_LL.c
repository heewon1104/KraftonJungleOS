...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	...
	ListNode *ll1Temp = ll1->head;
	ListNode *ll2Temp = ll2->head;
	// ll1 리스트가 빈 리스트일 때
	if (ll1Temp == NULL)
	{
		insertNode(ll1, 0, ll2Temp->item);
		removeNode(ll2, 0);
		return;
	}

	// ll1 리스트가 빈 리스트가 아닐 때
	else
	{
		int idx = 0;
		int list1size = ll1->size;
		ListNode *nextNode = ll2->head;

		while (idx < list1size && nextNode != NULL)
		{
			insertNode(ll1, idx * 2 + 1, nextNode->item);
